...
trait HasStreamPrefetchUnitHelper extends HasStreamPrefetchHelper{
  ...
  require(CELL_SIZE == 8 || CELL_SIZE == 64, "CELL_SIZE should be 8 or 64")
  require(BIT_VEC_WITDH == 16, "BIT_VEC_WITDH should be 16")
}
...
class StreamBitVectorArray(val CELL_SIZE: Int)(implicit p: Parameters) extends XSModule with HasStreamPrefetchUnitHelper {
  ...
  val s1_pf_l1_decr_vaddr = Cat(unit_to_cell_addr(s1_region_tag, s1_region_bits) - io.dynamic_depth, 0.U(CELL_OFFSET.W))
  val s1_pf_l2_incr_vaddr = Cat(unit_to_cell_addr(s1_region_tag, s1_region_bits) + (io.dynamic_depth << ratio), 0.U(CELL_OFFSET.W))
  val s1_pf_l2_decr_vaddr = Cat(unit_to_cell_addr(s1_region_tag, s1_region_bits) - (io.dynamic_depth << ratio), 0.U(CELL_OFFSET.W))
  val s1_pf_l3_incr_vaddr = Cat(unit_to_cell_addr(s1_region_tag, s1_region_bits) + (io.dynamic_depth << l3_ratio), 0.U(CELL_OFFSET.W))
  val s1_pf_l3_decr_vaddr = Cat(unit_to_cell_addr(s1_region_tag, s1_region_bits) - (io.dynamic_depth << l3_ratio), 0.U(CELL_OFFSET.W))
  // TODO: remove this
